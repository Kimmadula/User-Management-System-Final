...
export class FakeBackendInterceptor implements HttpInterceptor {
  ...
  private syncUsersWithAccounts() {
    try {
      ...
      console.log(
        "Stored accounts:",
        storedAccounts.map((a) => ({ id: a.id, email: a.email, role: a.role })),
      )

      // First, make sure our hardcoded users match what's in the accounts
      storedAccounts.forEach((account) => {
        const matchingUser = this.users.find((u) => u.email === account.email)
      ...
      console.log(
        "- Updated accounts:",
        accounts.map((a) => ({ id: a.id, email: a.email, role: a.role })),
      )
    } catch (error) {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    function getEmployeeWorkflows() {
      return authorize(null, () => {
        const employeeId = Number.parseInt(url.split("/").pop()!)
    ...
    function updateAccount() {
      ...
      const params = body
      const account = accounts.find((x) => x.id === idFromUrl()) // user accounts can update own profile and admin accounts can update all profiles
      if (account.id !== currentAccount().id && !isAuthorized(Role.Admin)) {
        return unauthorized()
      }
    ...
    function notFound() {
      return throwError({ status: 404, error: { message: "Not Found" } }).pipe(
        materialize(),
        delay(500),
        dematerialize(),
      )
    ...
    function authorize(requiredRole, callback) {
      ...
      if (requiredRole && user.role !== requiredRole) {
        console.error(
          `FAKE BACKEND - Authorization failed: User role ${user.role} does not match required role ${requiredRole}`,
        )
      ...
      try {
        ...
        if (token.startsWith("fake-jwt-token.")) {
          try {
            ...
            console.log(
              "FAKE BACKEND - Available accounts:",
              accounts.map((a) => ({ id: a.id, email: a.email, role: a.role })),
            )
            const account = accounts.find((x) => x.id === decodedPayload.id)
    ...
    function idFromUrl() {
      const urlParts = url.split("/")
      return Number.parseInt(urlParts[urlParts.length - 1])
    }
